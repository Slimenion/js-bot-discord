...
client.registry
  ...
  .registerGroups([
    ['music', ':notes: Музыкальные команды:'],
    ['gifs', ':film_frames: Gif команды:'],
    ['other', ':loud_sound: Остальные команды:'],
    ['guild', ':gear: Административные команды:']
  ])
...
client.once('ready', () => {
  ...
  client.user.setActivity('Постройку плотины');
...
client.on('guildMemberAdd', async member => {
  ...
  if (welcomeMsgSettings.status == 'yes') {
    ...
    if (welcomeMsgSettings.topImageText == 'default') {
      ...
      ctx.fillText(
        `Добро пожаловать в  ${member.guild.name}`, //<-- didn't play nice being stored in DB -Default
        canvas.width / 2.5,
        canvas.height / 3.5
      );
    ...
    var embed = new MessageEmbed()
      ...
      .setFooter(`Напиши help для списка команд!`)
      .setTimestamp();
    if (welcomeMsgSettings.embedTitle == 'default') {
      embed.setTitle(
        `:speech_balloon: Эй ${member.displayName}, ты кажется новенький в ${member.guild.name}!` //<-- didn't play nice being stored in DB -Default
      );
