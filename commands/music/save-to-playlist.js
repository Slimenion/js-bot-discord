...
module.exports = class SaveToPlaylistCommand extends Command {
  constructor(client) {
    super(client, {
      ...
      guildOnly: true,
      description: 'Сохраняет песню или плейлист в сохраненный плейлист',
      args: [
        {
          key: 'playlist',
          prompt: 'В какой плейлист ты хочешь сохранить?',
          type: 'string'
        },
        {
          key: 'url',
          prompt:
            'Какой URL ты хочешь сохранить в плейлист? Это также может быть URL на плейлист',
          type: 'string',
  ...
  async run(message, { playlist, url }) {
    ...
    if (!dbUserFetch) {
      message.reply('У тебя 0 плейлистов');
      return;
    }
    ...
    if (found) {
      ...
      if (Array.isArray(processedURL)) {
        ...
        message.reply('Предоставленный вами плейлист успешно сохранен');
      } else {
      ...
      message.reply(`У тебя нет плейлиста с именем ${playlist}`);
      return;
    }
  }
  ...
  static async processURL(url, message) {
    if (url.match(/^https?:\/\/(www.youtube.com|youtube.com)\/playlist(.*)$/)) {
      const playlist = await youtube.getPlaylist(url).catch(function() {
        message.reply(':x: Плейлист либо частный, либо не существует');
        return;
      });
      const videosArr = await playlist.getVideos().catch(function() {
        message.reply(
          ':x: Не удалось получить одно из видео в плейлисте'
        );
        return;
      });
      let urlsArr = [];
    ...
    const video = await youtube.getVideoByID(id).catch(function() {
      message.reply(':x: Возникла проблема с получением предоставленного вами видео');
      return;
    });
    if (video.raw.snippet.liveBroadcastContent === 'live') {
      message.reply("Я не поддерживаю прямые трансляции");
      return false;
    }
